...
def textchange(text):
    text = text.replace("100 Trying", "100 SKUSAM")
    text = text.replace("180 Ringing", "180 ZVONIM")
    text = text.replace("200 Ok", "200 VSETKO VPORIADKU")
    text = text.replace("408 Request Timeout", "408 DOSIEL CAS")
    text = text.replace("486 Busy here", "486 ZANEPRAZDNENY")
    text = text.replace("603 Decline", "603 ODMIETNUTE")
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 VSETKO VPORIADKU")
                return
        else:
            now = int(time.time())
            validity = now + expires

        logging.info("REGISTER From: %s - Contact: %s" % (fromm, contact))
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            logging.info("Ringing From '{0}' To '{1}'".format(origin, destination))
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 NEDOSTUPNY NA NEURCITY CAS")
        else:
    ...
    def processAck(self):
        ...
        origin = self.getOrigin()
        if len(destination) > 0:
            logging.info("Answered From '{0}'" .format(destination))
    ...
    def processNonInvite(self):
        ...
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info(" Ended From '{0}'" .format(origin))
            if destination in registrar and self.checkValidity(destination):
                socket, claddr = self.getSocketInfo(destination)
                # self.changeRequestUri()
                self.data = self.addTopVia()
                ...
                self.sendResponse("406 NEAKCEPTOVANY")
        else:
            self.sendResponse("500 CHYBA SERVERA")
